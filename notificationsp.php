...
<head>
    ...
    <style>
        ...
        .urgent-donation .details {
            font-size: 16px;
            font-weight: bold;
        }
        .urgent-donation .date {
            font-size: 14px;
            color: #555;
        }
    </style>
...
<body>
   ...
   <script>
        ...
        function fetchUrgentDonations() {
            var xhr = new XMLHttpRequest();
            xhr.open("GET", "fetch_urgent_donations.php", true);
            xhr.onload = function() {
                if (xhr.status == 200) {
                    var urgentDonations = JSON.parse(xhr.responseText);
                    displayUrgentDonations(urgentDonations);
                }
            };
            xhr.send();
        }

        function displayUrgentDonations(urgentDonations) {
            var container = document.querySelector(".container");
            container.innerHTML = ""; // Clear any existing content

            urgentDonations.forEach(function(donation) {
                var donationDiv = document.createElement("div");
                donationDiv.className = "urgent-donation";

                var details = document.createElement("p");
                details.textContent = donation.donation_details;
                details.style.margin = "0 0 5px";

                var date = document.createElement("p");
                date.textContent = "Date: " + donation.posted_on;
                date.style.margin = "0 0 5px";

                donationDiv.appendChild(details);
                donationDiv.appendChild(date);
